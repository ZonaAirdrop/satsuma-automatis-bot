...
async def display_balances(w3, config, account_address):
    try:
        ...
        print(f"{colors.reset}")
        print(f"{colors.white}Token{' ' * 15}Balance{' ' * 10}Symbol{colors.reset}")
        print("-" * 40)
        
        for token, info in balances.items():
            print(f"{colors.white}{token.ljust(20)}{colors.green}{info['formatted']:.6f}{' ' * 10}{colors.cyan}{info['symbol']}{colors.reset}")
...
async def main():
    ...
    while True:
        try:
            choice = input(f"{colors.white}[➤] Select option (1-10): {colors.reset}")
            ...
            if choice == "1":
                ...
                try:
                    count = int(input(f"{colors.cyan}Enter transaction count: {colors.reset}"))
                ...
                token_in = input(f"{colors.cyan}Token in (usdc/wcbtc/suma): {colors.reset}").lower()
                token_out = input(f"{colors.cyan}Token out (usdc/wcbtc/suma): {colors.reset}").lower()
                
                try:
                    amount = float(input(f"{colors.cyan}Amount: {colors.reset}"))
                ...
                token_a = input(f"{colors.cyan}Token A (usdc/wcbtc/suma): {colors.reset}").lower()
                token_b = input(f"{colors.cyan}Token B (usdc/wcbtc/suma): {colors.reset}").lower()
                
                try:
                    amount_a = float(input(f"{colors.cyan}Amount {token_a.upper()}: {colors.reset}"))
                    amount_b = float(input(f"{colors.cyan}Amount {token_b.upper()}: {colors.reset}"))
                ...
                try:
                    amount = float(input(f"{colors.cyan}SUMA amount: {colors.reset}"))
                    lock_days = int(input(f"{colors.cyan}Lock days: {colors.reset}"))
                    lock_time = lock_days * 24 * 60 * 60
                ...
                try:
                    amount = float(input(f"{colors.cyan}veSUMA amount: {colors.reset}"))
                ...
                gauge_addr = input(f"{colors.cyan}Gauge address: {colors.reset}")
                try:
                    weight = int(input(f"{colors.cyan}Vote weight: {colors.reset}"))
                ...
                print(f"{colors.cyan}Total Transactions:{colors.reset} {settings['total_transactions']}")
                print(f"{colors.green}Successful:{colors.reset} {settings['successful_transactions']}")
                print(f"{colors.red}Failed:{colors.reset} {settings['failed_transactions']}")
                print(f"{colors.blue}Last Transaction:{colors.reset} {settings.get('last_transaction_time', 'None')}")
